...
int sys_write_block(int block, const uint8_t *buf, uint32_t size) {
    if (size != BLOCK_SIZE) return -1;
    return io_write_block(block, buf);
}

int sys_read_block(int block, uint8_t *buf, uint32_t size) {
    if (size != BLOCK_SIZE) return -1;
    return io_read_block(block, buf);
}
